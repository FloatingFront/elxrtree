defmodule Elxrtree.CLI do
  ...
  def handle_struct(
    ...
    case dir_level do
      1 ->
        IO.puts("#{CssStyle.apply_list_items_root_directory(name)}")
        # IO.puts("#{CssStyle.apply_unordered_list_begin}")
        IO.puts("<!-- dir_level(--=1): #{dir_level} -->")

      # IO.puts("#{CssStyle.apply_div_begin_dir("elxrtree-dir",name)}")
      _ ->
  ...
  def handle_args(options) do
    ...
    case parsed do
      ...
      [] -> :help
      # valid arguments options should never reach here
      [_, _] -> :help
    end
  end
